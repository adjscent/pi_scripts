...
if [[ -e $IMG ]]; then
  ...
  losetup /dev/loop100 $IMG -o $(($P_START * 512)) --sizelimit $P_SIZE
  fsck -f /dev/loop100
  resize2fs -M /dev/loop100 # Make the filesystem as small as possible
  fsck -f /dev/loop100
  P_NEWSIZE=$( dumpe2fs /dev/loop100 2>/dev/null | grep '^Block count:' | awk '{print $3}' ) # In 4k blocks
